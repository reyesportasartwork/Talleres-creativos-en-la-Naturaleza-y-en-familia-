...
export const DynamicHeroBanner: React.FC<DynamicHeroBannerProps> = ({ assets, onOpenContact }) => {
  ...
  const slides = [
    ...
    {
      id: 'creaciones',
      tabLabel: language === 'es' ? '✦ Lo Creado en el Taller' : '✦ What We Create',
      tag: language === 'es' ? 'Galería de Creaciones' : 'Creations Showcase',
      title: language === 'es' ? 'Lo Creado en The Creative Forest' : 'Created in The Creative Forest',
      subtitle: language === 'es' ? 'Proyectos reales de arte y cuentos' : 'Real art & story projects',
      desc:
        language === 'es'
          ? 'Los niños no solo escuchan español: crean cómic, acuarelas, diarios botánicos y esculturas de papel, construyendo recuerdos inolvidables.'
          : 'Children do not just listen to Spanish: they create comics, watercolors, botanical journals, and paper crafts, forging lasting memories.',
      badgeHighlight: language === 'es' ? 'El niño aprende creando' : 'Learning by creating',
    },
  ];
